...
export class DocumentIntegrityTools {
    ...
    async detectDocumentTampering(args: z.infer<typeof DocInputSchema>): Promise<ToolResult> {
        try {
            ...
            // Simulated metadata analysis results
            // (In production: const pdfDoc = await PDFDocument.load(buffer); const info = pdfDoc.getTitle() etc.)
            const metadata = {
                creationDate: '2023-06-15T10:30:00Z',
                modificationDate: '2024-01-10T14:22:00Z',
                producer: docRef.includes('DIGITAL') ? 'Adobe Photoshop CC' : 'Government of India e-Filing System',
                creator: docRef.includes('STEEL') ? 'Microsoft Word 2019' : 'MCA Portal',
                pageCount: 1,
            };
